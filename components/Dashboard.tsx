...
import { Upload, FileCode, Play, Eye, Terminal, Save, CheckCircle, FileUp, Shield, Gift, Link as LinkIcon, Copy, AlertCircle, Loader2, Lock, Cpu, ArrowRight } from 'lucide-react';

interface DashboardProps {
  onSimulate: (mode: 'executor' | 'browser', script: ScriptData) => void;
  onNavigateToObfuscator: () => void; // New prop
}

const Dashboard: React.FC<DashboardProps> = ({ onSimulate, onNavigateToObfuscator }) => {
  ...
  const handleObfuscate = () => {
    ...
    // Simulate complex obfuscation delay
    setTimeout(() => {
      ...
      <nav className="border-b border-zinc-800 bg-zinc-900/50 backdrop-blur-md sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
             ...
             <button 
               onClick={onNavigateToObfuscator}
               className="hidden md:flex items-center gap-2 text-xs font-bold text-amber-500 bg-amber-500/10 border border-amber-500/20 px-4 py-1.5 rounded-full hover:bg-amber-500/20 transition-all"
             >
               <Cpu size={14} />
               OPEN IRONBREW
             </button>
             
             <div className="flex items-center gap-2 text-xs font-mono text-zinc-500 bg-zinc-900 border border-zinc-800 px-3 py-1.5 rounded-full">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 flex flex-col gap-4">
            <div className="bg-[#121215] border border-zinc-800 rounded-xl overflow-hidden shadow-2xl flex flex-col h-[600px] relative">
              {/* Overlay for Obfuscation */}
              {isObfuscating && (
                <div className="absolute inset-0 bg-black/80 backdrop-blur-sm z-50 flex flex-col items-center justify-center gap-4 animate-in fade-in">
                  <div className="w-16 h-16 border-4 border-red-900 border-t-red-500 rounded-full animate-spin"></div>
                  <div className="text-red-500 font-mono text-sm animate-pulse">APPLYING IRONCLAD PROTECTION...</div>
                </div>
              )}
                ...
                <div className="flex items-center gap-2">
                  ...
                  {!isProtected ? (
                    <button 
                      ...
                      className="flex items-center gap-2 px-3 py-1.5 text-xs font-medium bg-zinc-100 text-black hover:bg-white rounded-md transition-all shadow-lg hover:shadow-xl"
                    >
                      <Lock size={14} />
                      Protect Script
                    </button>
                    ...
                    >
                      ...
                      {generatedLink ? 'Deployed' : 'Publish Script'}
                    </button>
                  )}
                </div>
              </div>
            ...
            {apiError && (
              <div className="bg-red-950/10 border border-red-900/30 rounded-xl p-4 flex items-center gap-3 text-red-400 animate-in fade-in">
                <AlertCircle size={18} />
                <span className="text-sm font-mono break-all">{apiError}</span>
              </div>
            )}
          </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div 
              onClick={onNavigateToObfuscator}
              className="group bg-gradient-to-br from-amber-950/20 to-zinc-900 border border-amber-900/20 hover:border-amber-600/40 rounded-xl p-5 cursor-pointer transition-all hover:shadow-lg hover:shadow-amber-900/10"
            >
               <div className="flex items-center justify-between mb-2">
                  ...
                  <ArrowRight size={14} className="text-zinc-600 group-hover:text-amber-500 transition-colors transform group-hover:translate-x-1" />
               </div>
               <h3 className="text-lg font-bold text-zinc-200 mb-1">IronBrew Engine</h3>
               <p className="text-xs text-zinc-500">
                  Advanced virtualization and control flow flattening for maximum protection.
               </p>
            </div>
                  ...
                  <div className="space-y-2">
                     ...
                     <div className="flex gap-2">
                        <input 
                          ...
                          className="bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-sm w-full focus:outline-none focus:border-purple-500/50 transition-colors placeholder:text-zinc-700"
                        />
                        <button 
                     ...
                     <p className="text-[10px] text-zinc-600">Code: "NameLessFree"</p>
                  </div>
                ) : (
                  <div className="space-y-2 animate-in fade-in">
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        placeholder="Zee-Hub" 
                        className="bg-zinc-900 border border-zinc-800 rounded-lg pl-12 pr-3 py-2 text-sm w-full focus:outline-none focus:border-purple-500/50 transition-colors text-white placeholder:text-zinc-700"
                      />
                    </div>
                  </div>
                )}
                  ...
                  <div className="flex items-center justify-between relative z-10">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        <div className="font-medium text-zinc-200">Executor View</div>
                        <div className="text-[10px] text-zinc-500 font-mono">Simulates success access</div>
                      </div>
                    </div>
                ...
                >
                  ...
                  <div className="flex items-center justify-between relative z-10">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        <div className="font-medium text-zinc-200">Browser View</div>
                        <div className="text-[10px] text-zinc-500 font-mono">Simulates 403 denied</div>
                      </div>
                    </div>
